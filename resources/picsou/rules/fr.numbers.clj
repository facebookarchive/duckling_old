(
  ...
  {:dim :number
   :integer true
   :val (get {"zéro" 0 "zero" 0 "un" 1 "une" 1 "deux" 2 "trois" 3 "quatre" 4 "cinq" 5
  ...
  {:dim :number
   :integer true
   :val (get {"vingt" 20 "trente" 30 "quarante" 40 "cinquante" 50 "soixante" 60}
             (-> %1 :groups first .toLowerCase))}
  ...
  "numbers 21 31 41 51"
  [(integer 20 50  #(#{20 30 40 50} (:val %))) #"(?i)et" (integer 1 1)]
  {:dim :number
   :integer true
   :val (+ (:val %1) (:val %3))}

  "numbers 22..29 32..39 .. 52..59"
  [(integer 20 50 #(#{20 30 40 50} (:val %))) (integer 2 9)]
  {:dim :number
   :integer true
   :val (+ (:val %1) (:val %2))}

  "numbers 61 71"
  [(integer 60 60) #"(?i)-?et-?" (integer 1 11 #(#{1 11} (:val %)))]
  {:dim :number
   :integer true
   :val (+ (:val %1) (:val %3))}

  "numbers 81 91"
  [(integer 80 80) (integer 1 11 #(#{1 11} (:val %)))]
  {:dim :number
   :integer true
   :val (+ (:val %1) (:val %2))}

  "numbers 62..69 .. 92..99"
  [(integer 60 80 #(#{60 80} (:val %))) (integer 2 19)]
  {:dim :number
   :integer true
   :val (+ (:val %1) (:val %2))}
