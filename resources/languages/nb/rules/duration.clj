...
(
  ...
  "a <duration>"
  [#"(?i)(om )?en|ett|én|et" (dim :duration)]
  (in-duration (:value %2))
  ...
  [(integer 0) (dim :unit-of-duration) #"(?i)mere|mer"]; would need to add fields at some point
  {:dim :duration
   :value (duration (:grain %2) (:value %1))}
  ...
  [#"(\d+)\,(\d+)" #"(?i)timer?"] ;duration can't be negative...
  {:dim :duration
  ...
  [(integer 0) #"(?i)og (en )?halv time?"] ;duration can't be negative...
  {:dim :duration
   :value (duration :minute (+ 30 (* 60 (:value %1))))}

  "a <unit-of-duration>"
  [#"(?i)en|ett|et?" (dim :unit-of-duration)]
  {:dim :duration
