...
(
  ...
  [(integer 0) (dim :unit-of-duration) #"(?i)mere|mindre"]; would need to add fields at some point
  {:dim :duration
   :value (duration (:grain %2) (:value %1))}
  ...
  [(integer 0) #"(?i)og (en )?halv timer?"] ;duration can't be negative...
  {:dim :duration
   :value (duration :minute (+ 30 (* 60 (:value %1))))}

  "a <unit-of-duration>"
  [#"(?i)en|et?" (dim :unit-of-duration)]
  {:dim :duration
  ...
  [#"(?i)(omkring|cirka|ca.)" (dim :duration)]
  (-> %2
    (merge {:precision "approximate"}))

  "exactly <duration>" ; sharp
  [#"(?i)præcis" (dim :duration)]
  (-> %2
