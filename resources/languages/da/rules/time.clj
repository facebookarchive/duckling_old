(
  ...
  "constitution day";fifth of june in Denmark
  #"(?i)grundlov'?s?'? dag"
  (month-day 6 5)

  "Father's Day";fifth of june in Denmark
  #"(?i)far'?s?'? dag"
  (month-day 6 5)
  ...
  [#"(?i)(kommende|næste)" {:form :day-of-week}]
  ...
  "next <time>"
  [#"(?i)næste|kommende" (dim :time #(not (:latent %)))]
  (pred-nth-not-immediate %2 0)

  "last <time>"
  [#"(?i)(sidste|seneste)" (dim :time)]
  (pred-nth %2 -1)

  "<time> after next"
  [#"(?i)næste" (dim :time) #"(?i)igen"]
  (pred-nth-not-immediate %2 1)

   "<time> before last"
  [#"(?i)sidste" (dim :time) #"(?i)igen"]
  (pred-nth %2 -2)

  "last <day-of-week> of <time>"
  [#"(?i)sidste" {:form :day-of-week} #"(?i)af|i" (dim :time)]
  (pred-last-of %2 %4)

  "last <cycle> of <time>"
  [#"(?i)sidste" (dim :cycle) #"(?i)af|i" (dim :time)]
  (cycle-last-of %2 %4)

  ; Ordinals
  "nth <time> of <time>"
  [(dim :ordinal) (dim :time) #"(?i)af|i" (dim :time)]
  ...
  [{:form :time-of-day} #"(?i)(sharp|præcis)"]
  (-> %1
    (dissoc :latent)
  ...
  [#"(?i)præcis( kl.| klokken)?" {:form :time-of-day} ]
  (-> %2
    (dissoc :latent)
    (merge {:precision "exact"}))
  ...
  [(dim :time #(not (:latent %))) #"\-|til|indtil" (dim :time #(not (:latent %)))]
  (interval %1 %3 true)

  "from <datetime> - <datetime> (interval)"
  [#"(?i)fra" (dim :time) #"\-|til|indtil" (dim :time)]
  (interval %2 %4 true)

  "between <datetime> and <datetime> (interval)"
  [#"(?i)mellem" (dim :time) #"og" (dim :time)]
  (interval %2 %4 true)

  ; Specific for time-of-day, to help resolve ambiguities

  "<time-of-day> - <time-of-day> (interval)"
  [#(and (= :time-of-day (:form %)) (not (:latent %))) #"\-|til|indtil" {:form :time-of-day}] ; Prevent set alarm 1 to 5pm
  (interval %1 %3 true)

  "from <time-of-day> - <time-of-day> (interval)"
  [#"(?i)(efter|fra)" {:form :time-of-day} #"((men )?før)|\-|til|indtil" {:form :time-of-day}]
  (interval %2 %4 true)

  "between <time-of-day> and <time-of-day> (interval)"
  [#"(?i)mellem" {:form :time-of-day} #"og" {:form :time-of-day}]
  (interval %2 %4 true)

  ; Specific for within duration... Would need to be reworked
  "within <duration>"
  [#"(?i)indenfor" (dim :duration)]
  ...
  [#"(?i)i slutningen af" (dim :time)]
  (interval (cycle-nth :second 0) %2 true)

  ; One-sided Intervals

  "until <time-of-day>"
  [#"(?i)(engang )?inden|indtil|før|op til" (dim :time)]
  (merge %2 {:direction :before})

  "after <time-of-day>"
  [#"(?i)(engang )?efter" (dim :time)]
