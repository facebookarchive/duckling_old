(
  ...
  "valentine's day"
  #"(?i)valentine'?s?( dag| day)?"
  (month-day 2 14)

  "constitution day";June 6 in Sweden
  #"(?i)nationaldag(en)"
  (month-day 5 17)

  "Father's Day";second Sunday of November
  #"(?i)farsdag"
  (intersect (day-of-week 7) (month 11) (cycle-nth-after :week 1 (month-day 11 1)))

  "Mother's Day";second Sunday of February.
  #"(?i)morsdag"
  ...
  "now"
  #"(?i)precis nu|nu|(i )?detta ögonblick"
  (cycle-nth :second 0)
  ...
  "noon"
  #"(?i)lunch|(kl(\.|klockan)?)? tolv"
  (hour 12 false)

  "midnight|EOD|end of day"
  #"(?i)midnatt|mitt i natten|EOD"
  (hour 0 false)
  ...
  [{:form :time-of-day} #"(?i)(sharp|precis|prick)"]
  (-> %1
    (dissoc :latent)
    (merge {:precision "exact"}))

  "about <time-of-day>" ; about
  [#"(?i)(omkring|cirka|ca\.)( kl\.| klockan)?" {:form :time-of-day}]
  (-> %2
    (dissoc :latent)
    (merge {:precision "approximate"}))

  "exactly <time-of-day>" ; sharp
  [#"(?i)(exakt|prick)( kl.| klockan)?" {:form :time-of-day} ]
  (-> %2
    (dissoc :latent)
    (merge {:precision "exact"}))
  ...
  [ #"([012]?\d|30|31)(ter|\.)?" #"\-|till" #"([012]?\d|30|31)(ter|\.)?" {:form :month}]
  ...
  [(dim :time #(not (:latent %))) #"\-|till|till och med" (dim :time #(not (:latent %)))]
  (interval %1 %3 true)

  "from <datetime> - <datetime> (interval)"
  [#"(?i)från" (dim :time) #"\-|till|till och med" (dim :time)]
  (interval %2 %4 true)
  ...
  [#(and (= :time-of-day (:form %)) (not (:latent %))) #"\-|til och med|till" {:form :time-of-day}] ; Prevent set alarm 1 to 5pm
  (interval %1 %3 true)

  "from <time-of-day> - <time-of-day> (interval)"
  [#"(?i)(efter|från)" {:form :time-of-day} #"((men )?före)|\-|till och med|till" {:form :time-of-day}]
  (interval %2 %4 true)
  ...
  "until <time-of-day>"
  [#"(?i)(en gång )?innan|före|upptill" (dim :time)]
  (merge %2 {:direction :before})

  "after <time-of-day>"
  [#"(?i)(en gång )?efter" (dim :time)]
