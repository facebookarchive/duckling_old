...
(
  ...
  [#"(\d+)\,(\d+)" #"(?i)timmar?"] ;duration can't be negative...
  {:dim :duration
  ...
  [(integer 0) #"(?i)och (en )?halvtimme?"] ;duration can't be negative...
  {:dim :duration
  ...
  "<duration> from now"
  [(dim :duration) #"(?i)från (idag|nu|just nu)"]
  (in-duration (:value %1))

  "<duration> ago"
  [(dim :duration) #"(?i)(sen|sedan)"]
  ...
  [#"(?i)(omkring|cirka|ca.|ca|cirkus|runt)" (dim :duration)]
  (-> %2
    (merge {:precision "approximate"}))

  "exactly <duration>" ; sharp
  [#"(?i)(precis|exakt|prick)" (dim :duration)]
  (-> %2
